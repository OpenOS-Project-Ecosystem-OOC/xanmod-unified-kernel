#!/usr/bin/env bash
# packaging/ubuntu/install.sh — Ubuntu kernel install
#
# Two modes:
...
# ── APT mode (pre-built, x86-64 Ubuntu only) ──────────────────────────────────
...
echo "==> Ubuntu install complete. Reboot to use kernel ${KERNEL_VERSION}."
